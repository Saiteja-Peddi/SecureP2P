
# Contains details related to commonly used variable in multiple files. 

# Contains IP of names server of peer
pyroHost = "10.200.2.72"

# Contains IP of File index server.
fileIndexHost = "10.200.2.72"

# Contains IP of authentication server.
autheServerHost = "10.200.2.72"

authServerPort = 9000
peerPort = 7000
fileIndexPort = 9002

peerName = "example.peer"
fileIndexUri = "example.fileIndex"
authServerEncKey = 'WbabfbU8Sv6e9lZK9PJAvq0kFkVEaT_bLV_z0n8sBGk='
fileIndexEncKey = 'RVBcSKNpZ0ERx8-Zj_SeMCZj7WFV5mdCojSHW83DhDI='
peerCommEncKey = 'mgd-8MYUSmqHiijFf3UI1qfKKqt_iPGcfbjvLFiwvFI='


def main():
    print("Stores constants")

if __name__ == "__main__":
    main()